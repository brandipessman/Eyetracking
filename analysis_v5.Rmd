...
run_count <- aov_ez(id = "Subject",
                ...
                between = c("Gender", "PhobiaRank"))
summary(run_count)
emmeans(run_count, pairwise ~ image * Gender, adjust = "tukey")
Q1_image_rc_predictions <- data.frame(emmeans(run_count, ~ image))
Q1_ig_rc_predictions <- data.frame(question = "Q1", 
                            emmeans(run_count, ~ image:Gender)) %>% 
   separate(image, c("question", "photo"), sep = "_") %>% 
   mutate(photo = fct_relevel(photo, "Spider", "Butterfly")) %>% 
   dplyr::select(-lower.CL, -upper.CL)
```
...
dt <- data %>% 
  ...
  pivot_longer(cols = c(DT11_NonScorp:DT11_Scorpion), 
               names_to = "image", 
               values_to = "dwell_time") %>% 
  mutate(image = fct_recode(image, "Q6_NonScorp" = "DT11_NonScorp",
                                   "Q6_Scorpion" = "DT11_Scorpion"),
         image = fct_relevel(image, "Q6_Scorpion", "Q6_NonScorp"),
         Gender = factor(Gender),
         PhobiaRank = factor(PhobiaRank))
...
frdt <- data %>% 
  ...
  pivot_longer(cols = c(FRDT11_NonScorp:FRDT11_Scorpion), 
               names_to = "image", 
               values_to = "first_run_dwell_time") %>% 
  mutate(image = fct_recode(image, "Q6_NonScorp" = "FRDT11_NonScorp",
...
fft <- data %>% 
  ...
  pivot_longer(cols = c(FFT11_Non.Scorp:FFT11_Scorpion), 
               names_to = "image", 
               values_to = "first_fixation_time") %>% 
  mutate(image = fct_recode(image, "Q6_NonScorp" = "FFT11_Non.Scorp",
...
rc <- data %>% 
  ...
  pivot_longer(cols = c(RC11_NonScorp:RC11_Scorpion), 
               names_to = "image", 
               values_to = "run_count") %>% 
  mutate(image = fct_recode(image, "Q6_NonScorp" = "RC11_NonScorp",
...
Q6_ip_dt_predictions <- data.frame(emmeans(dwell, ~ image:PhobiaRank)) %>% 
  separate(image, c("question", "photo"), sep = "_") %>% 
   mutate(photo = fct_recode(photo, "Scorpion" = "Scorpion",
                            "Non-Scorpion Arachnid" = "NonScorp"),
         photo = fct_relevel(photo, "Scorpion", "Non-Scorpion Arachnid")) %>% 
  dplyr::select(-lower.CL, -upper.CL)
...
emmeans(dwell, pairwise ~ image * PhobiaRank, adjust = "tukey")
Q8_ip_dt_predictions <- data.frame(emmeans(dwell, ~ image:PhobiaRank)) %>% 
  separate(image, c("question", "photo"), sep = "_") %>% 
...
dt_predictions_SpiderVsNonSpider$photo <- factor(dt_predictions_SpiderVsNonSpider$image,
                        levels = c("Q1_Spider", "Q1_Butterfly", 
                                   ...
                                   "Q6_Scorpion", "Q6_NonScorp"),
                        labels = c("Spider", "Butterfly", 
                                   "Spider", "Insect", 
                                   "Spider Eyes", "Insect Eyes", 
                                   "Spider", "Non-Spider Arachnid", 
                                   "Spider", "Myriapod", 
                                   "Scorpion", "Non-Scorpion Arachnid"))
...
# creats a new label for the type of image so we can use it to color the graph
  mutate(spider_yn = ifelse(photo == "Spider" | photo == "Spider Eyes", "Spider", "Non-Spider"),
...
dt_predictions_features$photo <- factor(dt_predictions_features$image,
                        levels = c("Q7_Enlarged.Eyes", "Q7_Smaller.Eyes", 
                                   "Q8_More.color", "Q8_Less.color", 
                                   "Q9_Web", "Q9_No.web", 
                                   "Q10_Eggs", "Q10_No.eggs", 
                                   "Q11_Fangs", "Q11_No.fangs", 
                                   "Q12_Eating","Q12_Not.eating", 
                                   "Q13_Hairy", "Q13_Not.hairy"),
                        labels = c("Larger-Eyed Jumping Spider", "Smaller-Eyed Spider", 
                                   "Colorful Jumping Spider", "Neutral-Colored Jumping Spider", 
                                   "Spider On Web", "Spider On Ground", 
                                   "Spider With Eggs", "Spider Without Eggs",
                                   "Spider With Fangs", "Spider Without Fangs", 
                                   "Spider Eating", "Spider Not Eating", 
                                   "More Hairy Spider", "Less Hairy Spider"))
dt_predictions_features <- dt_predictions_features %>% 
  mutate(feature_yn = ifelse(photo == "Smaller-Eyed Spider"| photo == "Neutral-Colored Jumping Spider" | photo == "Spider On Ground" | photo == "Spider Without Eggs" | photo == "Spider Without Fangs" | photo == "Spider Not Eating" | photo == "Less Hairy Spider", "Without Feature", "With Feature"),
         feature_yn = fct_relevel(feature_yn, "With Feature", "Without Feature")) %>% 
  separate(image, c("question", "other"), remove = FALSE, sep = "_") %>% 
  mutate(question = fct_relevel(question, "Q7", "Q8", "Q9", "Q10", "Q11", "Q12", "Q13")) %>% 
...
scorpion <- scorpion %>% 
  separate(image, c("question", "photo"), sep = "_") %>% 
  mutate(photo = fct_recode(photo, "Non-Scorpion Arachnid" = "NonScorp"),
         photo = fct_relevel(photo, "Scorpion", "Non-Scorpion Arachnid"))

color <- color %>% 
  separate(image, c("question", "photo"), sep = "_") %>% 
  mutate(photo = fct_recode(photo, "Colorful Jumping Spider" = "More color", "Neutral-Colored Jumping Spider" = "Less color"),
         photo = fct_relevel(photo, "Colorful Jumping Spider", "Neutral-Colored Jumping Spider"))
...
# the rest of the layers edit the appearance of the graph
  scale_color_manual("", values = c("#af70d4", "#70b374")) + # set colors
  ...
  theme(legend.position = "top", # moves legend to the top
        axis.text.x = element_text(angle = 80, vjust = 1, hjust=1, # rotates x-axis text
                                   color = "black", size = 10),
...
dt_arachnid <- ggplot() +
  ...
  geom_point(position = position_dodge(width = 0.5),
             aes(x = photo, y = emmean, color = Gender), # the predicted mean
             size = 1.5,
             data = Q4_igp_dt_predictions) +
  geom_errorbar(position = position_dodge(width = 0.5),
                aes(x = photo, ymax = emmean + SE, ymin = emmean - SE, 
  ...
  labs(y = "Dwell Time (ms)", x = "") + # y axis label
  theme_classic() + # removes the default background/gridlines
  theme(legend.position = "top", # moves legend to the top
...
ggarrange(dt_arachnid, frdt_arachnid,
          nrow = 1, common.legend = TRUE, legend="top")
ggsave(here("figures_tables/spider_vs_arachnid.png"), width = 10, height = 7, scale = 1)
```
...
# spider vs butterfly, run count
# predictions: Q1_ig_rc_predictions
# raw data: butterfly
gender_butterfly_rc <- ggplot() +
  geom_jitter(aes(x = photo, y = run_count, color = Gender), # the raw data layer
              alpha = 0.1, width = 0.25, size = 1,
              data = butterfly) +
  geom_point(position = position_dodge(width = 0.5),
             aes(x = photo, y = emmean, color = Gender), # the predicted mean
             size = 1.5,
             data = Q1_ig_rc_predictions) +
  geom_errorbar(position = position_dodge(width = 0.5),
                aes(x = photo, ymax = emmean + SE, ymin = emmean - SE, 
                    color = Gender), # the predicted standard error 
                width = 0.5, linewidth = 0.75,
...
#ggarrange(gender_butterfly_dt, gender_butterfly_rc, gender_myriapod, gender_scorpion,
#          nrow = 1, common.legend = TRUE, legend="top")
legend <- cowplot::get_legend(gender_butterfly_dt)
p1 <- gender_butterfly_dt + theme(legend.position = "none")
p2 <- gender_butterfly_rc + theme(legend.position = "none")
p3 <- gender_myriapod + theme(legend.position = "none")
p4 <- gender_scorpion + theme(legend.position = "none")
plots_row <- cowplot::plot_grid(p1, p2, p3, p4, nrow = 1, align = "h", axis = "tb")
cowplot::plot_grid(legend, plots_row, ncol = 1, rel_heights = c(0.12, 1))
...
Q6_ip_dt_predictions <- Q6_ip_dt_predictions %>% 
  mutate(PhobiaRank = factor(PhobiaRank),
         PhobiaRank = fct_relevel(PhobiaRank, "Low", "Medium", "High"))
Q6_ip_frdt_predictions <- Q6_ip_frdt_predictions %>% 
  mutate(PhobiaRank = factor(PhobiaRank),
         PhobiaRank = fct_relevel(PhobiaRank, "Low", "Medium", "High"))
color <- color %>% 
  mutate(PhobiaRank = factor(PhobiaRank),
         PhobiaRank = fct_relevel(PhobiaRank, "Low", "Medium", "High"))
Q8_ip_dt_predictions <- Q8_ip_dt_predictions %>% 
  mutate(PhobiaRank = factor(PhobiaRank),
         PhobiaRank = fct_relevel(PhobiaRank, "Low", "Medium", "High"),
         photo = fct_recode(photo, "Colorful Jumping Spider" = "More color", "Neutral-Colored Jumping Spider" = "Less color"),
         photo = fct_relevel(photo, "Colorful Jumping Spider", "Neutral-Colored Jumping Spider"))
...
phobia_scorpion_dt <- ggplot() +
  geom_jitter(aes(x = photo, y = dwell_time, color = PhobiaRank), # the raw data layer
              ...
              data = scorpion) +
  geom_point(position = position_dodge(width = 0.5),
             aes(x = photo, y = emmean, color = PhobiaRank), # the predicted mean
             size = 1.5,
  ...
  scale_color_manual(values = c("#0068b7", "#fcc800", "#3d3d3d")) + # set colors
  xlab("") + # x axis label, I left it blank
  ylab("Dwell Time (ms)") + # y axis label
  theme_classic() + # removes the default background/gridlines
  theme(#legend.position = "top", # moves legend to the top
...
# more color vs less color jumping spider, dwell time
# predictions: Q8_ip_dt_predictions
# raw data: color
phobia_colorful_dt <- ggplot() +
  geom_jitter(aes(x = photo, y = dwell_time, color = PhobiaRank), # the raw data layer
              alpha = 0.1, width = 0.25, size = 1,
              data = color) +
  geom_point(position = position_dodge(width = 0.5),
...
cowplot::plot_grid(phobia_scorpion_dt, phobia_scorpion_frdt, phobia_colorful_dt, nrow = 1, align = "h")
#ggarrange(phobia_scorpion_dt, phobia_scorpion_frdt, phobia_colorful_dt,
#          nrow = 1, common.legend = TRUE, legend="top")
legend <- cowplot::get_legend(phobia_scorpion_dt)
p1 <- phobia_scorpion_dt + theme(legend.position = "none")
p2 <- phobia_scorpion_frdt + theme(legend.position = "none")
p3 <- phobia_colorful_dt + theme(legend.position = "none")
plots_row <- cowplot::plot_grid(p1, p2, p3, nrow = 1, align = "h", axis = "tb")
cowplot::plot_grid(legend, plots_row, ncol = 1, rel_heights = c(0.12, 1))
ggsave(here("figures_tables/imagexphobia.png"), width = 9, height = 7, scale = 1)
```
